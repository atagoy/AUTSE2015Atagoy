<div id="system">
	...
	<?php if (have_posts()) : ?>
		...
		<?php elseif (is_day()) : ?>
			<h1 class="title"><?php printf(__('Archive for %s', 'warp'), get_the_time('F jS, Y')); ?></h1>
		<?php elseif (is_month()) : ?>
			<h1 class="title"><?php printf(__('Archive for %s', 'warp'), get_the_time('F, Y')); ?></h1>
		<?php elseif (is_year()) : ?>
			<h1 class="title"><?php printf(__('Archive for %s', 'warp'), get_the_time('Y')); ?></h1>
