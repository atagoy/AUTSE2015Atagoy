	...
	*/
	public function __construct() {
		...
		if (defined('WP_ALLOW_MULTISITE') && WP_ALLOW_MULTISITE) {
			...
			$settings = count($settings) ? $settings : file_get_contents($this['path']->path('template:config.default'));
		} else {

			$file     = $this['path']->path('template:config');
			$settings = file_get_contents($file ? $file : $this['path']->path('template:config.default'));
		}
