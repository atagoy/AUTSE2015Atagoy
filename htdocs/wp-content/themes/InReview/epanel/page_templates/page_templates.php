...
function et_ptemplates_css(){
	if ( !is_admin() && !(strstr( $_SERVER['PHP_SELF'], 'wp-login.php')) ) {
		...
		echo('<link media="screen" type="text/css" href="'.ET_PT_PATH.'/js/prettyphoto/prettyPhoto.css" rel="stylesheet" />');
...
function et_ptemplates_footer_js(){
	if ( !is_admin() ) {
		...
		wp_enqueue_script('prettyphoto', ET_PT_PATH . '/js/prettyphoto/jquery.prettyPhoto.js', array('jquery'), '3.0.3', true);
		wp_enqueue_script('et-ptemplates-frontend', ET_PT_PATH . '/js/et-ptemplates-frontend.js', array('jquery','prettyphoto'), '1.1', true);
	}
}
...
function et_ptemplate_meta($callback_args) {
	...
	#var_dump($temp_array['et_fullwidthpage']);
	$et_fullwidthpage = isset( $temp_array['et_fullwidthpage'] ) ? (int) $temp_array['et_fullwidthpage'] : 0;
	$et_regenerate_numbers = isset( $temp_array['et_regenerate_numbers'] ) ? (int) $temp_array['et_regenerate_numbers'] : 0;
	$et_ptemplate_blogstyle = isset( $temp_array['et_ptemplate_blogstyle'] ) ? (int) $temp_array['et_ptemplate_blogstyle'] : 0;
	$et_ptemplate_showthumb = isset( $temp_array['et_ptemplate_showthumb'] ) ? (int) $temp_array['et_ptemplate_showthumb'] : 0;
