<?php
/**
 * List of directories that act as sources, all files in the folder
 * gets imported.  
 *
 * @package CSV 2 POST
 * @subpackage Views
...
class CSV2POST_Directorysources_View extends CSV2POST_View {
    ...
    protected $view_name = 'directorysources';
    ...
    */
    public function datatables( $data, $box ) { 
        ...
        // first we get all paths setup as directory sources
        // we query the sources table for data related to that directory and build a row of information
        if( isset( $csv2post_settings['directorysources'] ) && is_array( $csv2post_settings['directorysources'] ) ) {
            $i = 0;
            foreach( $csv2post_settings['directorysources'] as $dirpath => $dirsettings ) {

                $some_sources = $this->DB->selectwherearray( $wpdb->c2psources, "directory = '$dirpath'", 'progress,changecounter,tablename', '*' );
      
                $total_progress = 0;
                
                $total_changes = 0;
                
                foreach( $some_sources as $source ) {
                    $total_progress = $total_progress + $source['progress'];
                    $total_changes = $total_changes + $source['changecounter'];        
                }
            
                // add progress, add changes counter (to be helpful stats)
                $query_results[ $i ]['filesprocessed'] = count( $some_sources );
                            
                $query_results[ $i ]['path'] = $dirpath;
            
                if( !isset( $some_sources['tablename'] ) ) {
                    $query_results[ $i ]['databasetable'] = __( 'Unknown', 'csv2post' );    
                } else {
                    $query_results[ $i ]['databasetable'] = $some_sources['tablename'];
                }

                $query_results[ $i ]['progress'] = $total_progress;
                
                $query_results[ $i ]['monitordirectorychange'] = $dirsettings['monitor'];

                $query_results[ $i ]['name'] = $dirsettings['name'];

                ++$i;            
            }
        }
        
        $WPTableObject = new C2P_Directorysources_Table();
...
class C2P_Directorysources_Table extends WP_List_Table {
    ...
    function column_default( $item, $column_name ){
        ...
        switch( $column_name ){
            ...
            case 'progress':
                ...
                break;   
            case 'monitordirectorychange':
                return $item['monitordirectorychange'];    
                break;                                                                                
            default:
    ...
    function get_columns() {
        $columns = array(
            ...
            'name' => __( 'Name', 'csv2post' ),
            'path' => __( 'Path', 'csv2post' ),
            'filesprocessed' => __( 'Total Files Processed', 'csv2post' ), 
            'databasetable' => __( 'Database Table', 'csv2post' ),
            'progress' => __( 'Progress', 'csv2post' ),            
            'monitordirectorychange' => __( 'Monitor Directory', 'csv2post' ),
